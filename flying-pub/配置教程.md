...
| permission.isOnlyPubPrivate | 所有用户都可以免费阅读，但只有白名单里面的用户才能发文章                                       |
...
1. 打开 `config/config.js`
2. 修改 settings 里面的 `site.name`
3. 保存
4. 重启服务，运行 `./scripts/redeploy_flying_pub.sh`
5. 搞定

## 举例 2：我要修改站点 logo

1. 打开 `config/config.js`
2. 修改 settings 里面的 `site.logo`
3. 保存
4. 重启服务，运行 `./scripts/redeploy_flying_pub.sh`
5. 搞定
...
1. 打开 `config/config.js`
2. 修改 `settings` 里面的 `permission.isPrivate` 为 `true`
3. 修改 `auth` 里面的 `whitelist`（白名单），把你允许的用户的 mixin id 填到这里，假如 id 是 1095057，那这里就是 `mixin: [1095057]`
4. 保存
...
1. 打开 `config/config.js`
2. 修改 `auth` 里面的 `adminList`（管理员名单），把 mixin id 填到这里，假如 id 是 1095057，那这里就是 `mixin: [1095057]`
3. 保存
4. 重启服务，运行 `./scripts/redeploy_flying_pub.sh`
5. 搞定
