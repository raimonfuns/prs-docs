...
2. 打开 `config/config.js`
3. 修改 `settings` 里面的 `site.name`
4. 保存
5. 重启服务，运行 `./scripts/redeploy_flying_pub.sh`
6. 搞定
...
1. 进入飞帖目录 `cd flying-pub-setup`
2. 打开 `config/config.js`
3. 修改 `settings` 里面的 `permission.isPrivate` 为 `true`
4. 修改 `auth` 里面的 `whitelist`（白名单），把你允许的用户的 mixin id 填到这里，假如 id 是 1095057，那这里就是 `mixin: [1095057]`，记得把自己加上哦
5. 保存
...
1. 进入飞帖目录 `cd flying-pub-setup`
2. 打开 `config/config.js`
3. 修改 `auth` 里面的 `adminList`（管理员名单），把 mixin id 填到这里，假如 id 是 1095057，那这里就是 `mixin: [1095057]`
4. 保存
5. 重启服务，运行 `./scripts/redeploy_flying_pub.sh`
6. 搞定
...
1. 进入飞帖目录 `cd flying-pub-setup`
2. 运行 `./scripts/stop.sh`

## 举例 6：我要访问数据库（技术要求高，风险也高，谨慎操作）

1. 进入飞帖目录 `cd flying-pub-setup`
2. 运行 `./scripts/access_pg.sh`

## 举例 7：我要备份站点的数据

1. 进入飞帖目录 `cd flying-pub-setup`
2. 运行 `./scripts/backup.sh`
