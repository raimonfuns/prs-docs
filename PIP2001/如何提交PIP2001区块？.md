...
```
大部分字段和授权区块是一样的，下面我为你介绍一下不一样的字段

|字段|含义|
|-------------|-------------|
|meta.uris|这篇文章的储存地址，PRS 只储存哈希，不储存内容，这个地方就是指向文件储存的地方
|meta.mime|文件类型|
...
如果你的内容是私密的、或者是需要付费才能阅读的，你不想把明文公开发布到区块上面。你可以先加密内容，然后把加密后的文章发布到链上。
...
你把 `block.block_num` 更新到你数据库中的 block，从而表示这个 block 是已上链的了。

## 修改文章区块
由于区块不能修改，所以修改文章就是发布一个新区块，指向一个旧区块，起到了覆盖或者替换的效果。
...
```
大部分字段和新建文章区块是一样的，下面我为你介绍一下不一样的字段
...
## 如果一个用户发布了不合适的内容，想要禁止Ta，怎么做呢？
给他提交一个【解除授权区块】，我给你介绍一下如何提交这种区块。
